...
1. 해당 항목들을 확인합니다.
2. **원문 텍스트가 누락되었는지 반드시 확인합니다** — PDF의 모든 문단, 조항, 항목이 마크다운에 빠짐없이 포함되어야 합니다. 요약하거나 생략하지 않고 원문 그대로 변환해야 합니다.
3. 실제 누락인 경우 마크다운을 수정합니다.
4. 형식 차이(머리글/바닥글, 유니코드 따옴표 `"` vs `"`, 특수기호 등)인 경우 무시합니다.
